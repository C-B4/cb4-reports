...
def extractResponseCookies(self, headers):
...
class ResultFetcher:
    ...
    def isNumberValue(self, value):
        return isinstance(value, (int, float))

    # ----------------------------------------------------------------------------

    def isFloatingPointString(self, value):
        """
        ...
        if not self.isNumberString(intPart):
            return False
        ...
        if not self.isNumberString(facPart):
            return False
    ...
    def normalizeValue(self, value):
        ...
        if loCase == "none":
            ...
            return False
        elif self.isNumberString(loCase):
            return int(loCase)
        else:
            return value
    ...
    def putIfAbsent(self, args, key, value):
        if args is None:
            return { "key": value }
    ...
    def parseCommandLineArguments(self, args):
        ...
        for item in args:
            ...
            if sepPos < 0:
                ...
                propValue = self.normalizeValue(propPair[sepPos + 1:])
    ...
    def mergeCommandLineArguments(self, args, extraArgs):
        """
        ...
        for key,value in extraArgs.items():
            ...
            if originalValue is None:
                ...
                self.mergeCommandLineArguments(originalValue, value)

        return args

    # ----------------------------------------------------------------------------

    def replacePlaceholders(self, line, extraProps):
        """
    ...
    def splitByFixedWidth(self, s, width):
        if s is None:
            return None
    ...
    def loadJsonFile(self, configFile):
        if self.isEmpty(configFile):
            return {}
    ...
    # key/value pairs
    def buildUrlQueryParams(self, queryParams):
        if self.isEmpty(queryParams):
            return None
    ...
    def executeHttpRequest(self, url, verb, headersMap, reqData, logger, args, cookieJar=None, asJson=True):
        if headersMap is None:
            headersMap = {}

        dataBytes = None
        if not self.isEmpty(reqData):
    ...
    def downloadReport(self, url, verb, headersMap, reqData, logger, args):
        if headersMap is None:
            headersMap = {}
        ...
        try:
            ...
            filePath = self.resolveOutputFilePath(args)
    ...
    def resolveKeycloakRealmAccessUrl(self, args):
        if args is None:
            self.die("No Keycloak access arguments provided")

        protocol = args.get("protocol", "https")
        host = args.get("host", None)
        if self.isEmpty(host):
            self.die("No Keycloak host specified")
    ...
    def resolveKeycloakOpenidAccessUrl(self, args):
        return self.resolveKeycloakRealmAccessUrl(args) + "/protocol/openid-connect"

    # Returns a full URL
    def resolveKeycloakDirectTokenAccessUrl(self, args):
        return self.resolveKeycloakOpenidAccessUrl(args) + "/token"

    def resolveDirectGrantUserToken(self, username, password, clientId, logger, args):
        ...
        url = self.resolveKeycloakDirectTokenAccessUrl(args)
        rsp = self.executeHttpRequest(url, "POST", requestHeaders, formData, logger, args)
        return rsp["body"]
    ...
    #    }
    def resolveStandardUserTokenRequestParameters(self, clientId, logger, args):
        params = {}

        baseUrl = self.resolveKeycloakOpenidAccessUrl(args) + "/auth"
        ...
        url = baseUrl + "?" + self.buildUrlQueryParams(queryParams)
        params["referer"] = url

        rsp = self.executeHttpRequest(url, "GET", { "Accepts": "*/*" }, None, logger, args, asJson=False)
    ...
    #    }
    def resolveKeycloakSessionAuthenticationCode(self, username, password, clientId, params, logger, args):
        url = params['loginUrl']
        if self.isEmpty(url):
        ...
        if self.isEmpty(sessionId):
            self.die("No Keycloak session ID cookie present")
        cookies["AUTH_SESSION_ID"] = sessionId

        kcToken = params.get('KC_RESTART', None)
        if self.isEmpty(kcToken):
            self.die("No Keycloak restart cookie present")
        ...
        rsp = self.executeHttpRequest(url, "POST", requestHeaders, formData, logger, args, cookieJar=cookies, asJson=False)
        redirectHistory = rsp.get("history", None)
        if self.isEmpty(redirectHistory):
            self.die("No Keycloak redirection history available after successful authentication")
    ...
    def resolveStandardUserToken(self, username, password, clientId, logger, args):
        params = self.resolveStandardUserTokenRequestParameters(clientId, logger, args)
        redirectUri = urllib.parse.unquote(params['redirectUri'])

        params = self.resolveKeycloakSessionAuthenticationCode(
            username, password, clientId, params, logger, args)

        code = params.get("code", None)
        if self.isEmpty(code):
            self.die("No Keycloak access token session code")
        ...
        url = self.resolveKeycloakDirectTokenAccessUrl(args)
        rsp = self.executeHttpRequest(url, "POST", requestHeaders, formData, logger, args, asJson=True)
        return rsp["body"]

    # ----------------------------------------------------------------------------

    def resolveUserToken(self, username, password, logger, args):
        logger.info("Retrieving Keycloak access token")

        if isinstance(username, (int, float)):
            username = str(username)
        elif self.isEmpty(username):
            self.die("No Keycloak access username provided")

        if isinstance(password, (int, float)):
            password = str(password)
        elif self.isEmpty(password):
            self.die("No Keycloak access password provided")

        clientId = args.get("clientId", None)

        if self.isEmpty(clientId):
            self.die("No Keycloak client identifier provided")
        ...
        if self.isEmpty(accessToken):
            logger.error("No access token in Keycloak response: %s" % str(rsp))
            self.die("No access token returned from Keycloak")
    ...
    def resolveExportResponseReportAccessUrl(self, args):
        if args is None:
            self.die("No dashboard access arguments provided")

        protocol = args.get("protocol", "https")
        host = args.get("host", None)
        if self.isEmpty(host):
            self.die("No dashboard host specified")
    ...
    def buildDashboardRetrievalFilter(self, stateValue):
        if stateValue == 'ALL':
            return None
    ...
    def exportResponseReportCsv(self, accessToken, accessParams, logger, args):
        ...
        self.downloadReport(url, "POST", requestHeaders, reqData, logger, args);

    def resolveOutputFilePath(self, args):
        filePath = args.get("file", None)
        if not self.isEmpty(filePath):
            filePath = self.resolvePathVariables(filePath)
            if os.path.isabs(filePath):
                return filePath

        dirPath = args.get("dir", None)
        if not self.isEmpty(dirPath):
    ...
    # }
    def writeResultRow(self, fout, r, stateValue):
    ...
    def writeResultsToOutput(self, filePath, withHeaders, stateValue, rows, logger):
        numResults = len(rows)
        if numResults <= 0:
    ...
    def dumpResults(self, username, accessToken, accessParams, outputParams, logger, args):
        stateValue = accessParams.get("state", "ALL")
        stateValue = stateValue.upper()

        self.exportResponseReportCsv(accessToken, accessParams, logger, args)
        return

    # ----------------------------------------------------------------------------

    # Adds the protocol/host/port if provided and not already set
    def resolveEndpointAccessParameters(self, protocol, host, port, args):
        if not self.isEmpty(protocol):
            args = self.putIfAbsent(args, "protocol", protocol)
        if not self.isEmpty(host):
            args = self.putIfAbsent(args, "host", host)
        if port > 0:
            args = self.putIfAbsent(args, "port", port)
        return args
    ...
    def main(self, args=[], dict_args={}):
        if len(args) > 0:
            subArgs = self.parseCommandLineArguments(args)
        else:
            subArgs = dict_args

        print("Executing with args: {}".format(subArgs))

        configFile = subArgs.get("configFile", None)
        if not self.isEmpty(configFile):
            configFile = self.resolvePathVariables(configFile)
            if not os.path.isabs(configFile):
                dir = os.path.abspath(os.getcwd())
                configFile = os.path.join(dir, configFile)
                subArgs["configFile"] = configFile

            extraArgs = self.loadJsonFile(configFile)
            subArgs = self.mergeCommandLineArguments(subArgs, extraArgs)
    ...
    def run(self, dict_args=None):
        ...
        if dict_args:
            self.main(dict_args=dict_args)
        else:
            self.main(args=sys.argv[1:])
